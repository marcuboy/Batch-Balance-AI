...
export const exportToExcel = (
  ...
  if (understockParts.length > 0) {
    const understockData = understockParts.map(p => ({
      ...
      'Required Volume (m³)': (p as unknown as Record<string, number>)[`uv${horizon}`] || 0,
  ...
  if (parts.length > 0) {
    const allPartsData = parts.map(p => ({
      ...
      'Shortage Risk': p.shortageRisk,
      'Overstock Volume (m³)': (p as unknown as Record<string, number>)[`ov${horizon}`] || 0,
      'Shortage Volume (m³)': (p as unknown as Record<string, number>)[`uv${horizon}`] || 0,
      'Value (GBP)': (p as unknown as Record<string, number>)[`osVal${horizon}`] || 0,
